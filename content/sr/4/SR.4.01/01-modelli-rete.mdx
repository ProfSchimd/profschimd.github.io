---
title: Modelli di riferimento per le reti
---

## Modelli a strati
Una rete come **Internet** è un *sistema* estremamente complesso, si [stima](https://techjury.net/blog/how-many-iot-devices-are-there/) che ci siano *decide di miliardi* di dispositivi connessi alla rete (quanti vostri dispositivi sono connessi?). 

Per gestire un sistema così complesso le reti sono basate su **architetture a strati** (*layered architecture*). 

> [!note]Definizione
> Un'**architettura di rete** è l'insieme di tutte le componenti, hardware e software, che costituiscono la rete e le modalità con cui tali componenti sono organizzate e comunicano tra di loro. 
...
5. **Sessione** (*session*): gestisce le sessioni di comunicazione tra applicazioni
6. **Presentazione** (*presentation*): traduzione, codifica, compressione e cifratura dei dati 
7. **Applicazione** (*application*): fornisce i servizi di rete  alle applicazioni
...
<StackedTooltip data={[
    { name: "Fisico", description: "Fisico (physical): trasmette bit sul canale di trasmissione" },
    { name: "Collegamento", description: "Collegamento (data link): crea e trasmette frame" },
    { name: "Rete", description: "Rete (network): crea e instrada pacchetti" },
    { name: "Trasporto", description: "Trasporto (transport): crea e trasmette segmenti" },
    { name: "Sessione", description: "Sessione (session): gestisce le sessioni di comunicazione tra applicazioni" },
    { name: "Presentazione", description: "Presentazione (presentation): traduzione, codifica, compressione e cifratura dei dati " },
    { name: "Applicazione", description: "Applicazione (application): fornisce i servizi di rete  alle applicazioni" },
  ]}/>

### Modello TCP/IP
![Corrispondenza tra modello ISO/OSI e modello TCP/IP](https://www.guru99.com/images/1/102219_1135_TCPIPvsOSIM1.png)
