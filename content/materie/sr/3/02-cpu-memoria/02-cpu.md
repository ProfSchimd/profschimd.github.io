...
La **CPU** (**Central Processing Unit** - *unità centrale di elaborazione*) rappresenta il "cervello" di un calcolatore ed esegue le **istruzioni** che compongono un *programma*. Tali istruzioni sono molto semplici: somma due numeri, memorizza un numero in memoria, leggi un carattere dalla tastiera, ... 

L'insieme delle istruzioni che una CPU è in grado di eseguire si chiama **Instruction Set Architecture ISA** (ISA), in italiano *repertorio di istruzioni*, che dipende dal modo in cui la CPU è organizzata internamente (**micro-architettura**). Ad esempio, se una CPU non ha un circuito per dividere due numeri, la CPU non sarà in grado di fare le divisioni in un'unica operazione. Sarà quindi necessario scrivere una sequenza di istruzioni (un *algoritmo*) per la divisione utilizzando solo operazioni di cui la CPU dispone.
...
Nella figura sotto, si vede uno schema semplificato di CPU contenente le varie parti descritte sopra.
...
Vediamo più in dettaglio come la CPU esegue il *fetch-and-execute* attivando opportunamente i componenti che sono stati descritti sopra. Negli esempi si userà l'istruzione assembly `add R1, [0x3D]` che significa: somma il valore nel registro `R1` al valore nella cella di memoria di indirizzo `0x3D` e metti il risultato di questa somma nel registro `R1`.

### Fetch dell'istruzione
Le istruzioni che la CPU esegue si trovano tutte nella memoria centrale (la RAM), la CPU deve recuperarle prima di poterle eseguire. Per fare questo, la CPU utilizza due registri: il **Program Counter, PC**  (*contatore di programma*) e l'**Instruction Register** (*registro di istruzione*). Inoltre, dovendo accedere alla memoria, la CPU utilizza anche i registri `MAR` ed `MDR` il cui funzionamento è spiegato in [questa pagina]({{<ref "01-ram" >}}).
...
2. Il valore che la RAM legge finisce nel bus dati prima e nel `MDR` poi.
3. Il valore del `MDR` viene copiato nell'`IR`.
4. Il valore del PC viene incrementato opportunamente in modo che sia pronto per la prossima istruzione (per ISA a 64 bit l'incremento è di 8).

{{<column/two-cols wl="6" wr="6" content="left" embed="img/cpu_fetch_esempio.html">}}
#### Esempio di fetch
Il `PC` contiene l'indirizzo `OxA1` che arriva alla RAM attraverso il `MAR` ed il bus indirizzi (passo 1 del *fetch*). La RAM recupera il valore (in questo caso `0x04`) che viene copiato nel `MDR` (passo 2 del *fetch*). A questo punto il valore dal `MDR` viene copiato nell'IR (passo 3 del *fetch*) ed il PC viene incrementato (di 1 nella figura) per il *fetch* della prossima istruzione (passo 4 del *fetch*).
{{</column/two-cols>}}

### Decodifica dell'istruzione
Dal momento che una CPU può eseguire diversi tipi di istruzione (somma, sottrazione, ...), è necessario un meccanismo per "decidere" quale operazione eseguire in base all'istruzione recuperata dalla RAM durante il *fetch*. 
...
Per le operazioni che producono un risultato, ad esempio la somma di due numeri, potrebbe essere necessario memorizzarlo nella memoria RAM, questo avviene nella fase di *write*. In questa fase, la CPU utilizza i registri `MAR` e `MDR` per spostare il risultato dal registro dove lo ha messo la ALU alla memoria. È importante notare che questa operazione può non essere richiesta se, ad esempio, non si vuole che il risultato venga messo nella RAM.
