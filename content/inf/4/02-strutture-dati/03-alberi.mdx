...
Le strutture dati ad albero sono usate per rappresentare informazioni con *struttura gerarchica*, esempi sono l'albero genealogico e l'organizzazione di file e cartelle all'interno del *filesystem*.

In tutti questi casi, la migliore struttura per memorizzare i dati è quella ad **albero**. Gli alberi sono anche utilizzati anche come struttura per favorire la *ricerca* ed anche in ambiti come il *machine learning* e l'*intelligenza artificiale*.
...
> [!tip]Osserva
> Le interfacce `ITreeNode` e `ITree` sopra sono state scritte in modo un albero sia identificato dalla sua radice. Una volta ottenuta la radice, è possibile navigare l'albero per raggiungere tutti qualsiasi nodo.
>
> In è possibile raggiungere tutti i nodi partendo da un nodo qualsiasi. Infatti è sempre possibile utilizzando anche collegamenti al genitore. 

> [!example]Esercizio
> Implementare in Java l'interfaccia `ITreeNode` descritta sopra mediante una classe concreta di nome `TreeNode`. Oltre ai metodi dell'interfaccia la classe deve prevedere un costruttore che imposti il *valore* (`value`) del node, il riferimento al *nodo genitore* (`parent`) e la *lista dei figli* (`children`). La firma del costruttore sarà quindi:
> ```java
...
L'albero contiene 9 nodi con 0, 1 e 2 figli, ma non ci sono nodi con 3 o più figli, quindi l'albero sopra è un albero binario. La radice dell'albero contiene il valore `53` ed ha due figli, il figlio sinistro contiene il valore `42`, il figlio destro contiene il valore `56`. Il nodo con valore `21` ha un solo figlio, dal disegno possiamo dedurre che si tratti del figlio destro.

Un nodo qualsiasi si può considerare radice di un sottoalbero, negli alberi binari per ogni nodo possiamo avere due sottoalberi uno con radice il figlio sinistro ed uno con radice il figlio destro. 
...
* `print(node)`: che mostra a console il valore (funzione di comodo al posto di  `System.out.println(node.vaule())`),
...
I due alberi sopra sono due alberi binari, ma solo quello più a destra è un
